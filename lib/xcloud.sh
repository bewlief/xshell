...
# login oc API server and set OC_TOKEN
# login
function oc::login() {
    local server=${1:-$OPENSHIFT_SERVER}
    local username=${2:-$OPENSHIFT_USERNAME}
    local password=${3:-$OPENSHIFT_PASSWORD}

    if [[ -z "$server" || -z "$username" || -z "$password" ]]; then
        echo "Error: OPENSHIFT_SERVER, OPENSHIFT_USERNAME, OPENSHIFT_PASSWORD must be set!"
        return 1
    fi

    oc login -u "$username" -p "$password" --server="$server" --insecure-skip-tls-verify >/dev/null 2>&1
    if [[ $? -eq 0 ]]; then
        ...
        if [[ -n "$TOKEN" ]]; then
            ...
            echo "Failed to extract the access token. Please check he user information."
        fi
    else
